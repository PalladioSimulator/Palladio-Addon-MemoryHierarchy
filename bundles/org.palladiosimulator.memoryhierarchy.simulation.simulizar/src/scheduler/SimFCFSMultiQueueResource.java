...
/**
 * Nearly identical to SimFCFSResource. Only the toNow() method is modified.
 * 
 * @author KTruong
 *
 */
public class SimFCFSMultiQueueResource extends AbstractActiveResource {
    ...
    public SimFCFSMultiQueueResource(final SchedulerModel model, final String name, final String id,
            final long capacity) {
    ...
    private void toNow() {
        final double now = getModel().getSimulationControl()
            .getCurrentSimulationTime();
        final double passed_time = now - last_time;
        if (MathTools.less(0, passed_time)) {
            int counter = 0;
            int capacity = (int) getCapacity();
            for (Iterator<ISchedulableProcess> iterator = processQ.iterator(); iterator.hasNext()
                    && counter < capacity;) {
                ISchedulableProcess iSchedulableProcess = (ISchedulableProcess) iterator.next();
                counter++;
                if (iSchedulableProcess != null) {
                    double demand = running_processes.get(iSchedulableProcess);
                    demand -= passed_time;
...
//	                 avoid trouble caused by rounding issues
                    ...
                    running_processes.put(iSchedulableProcess, demand);
                }
            }
        }
        last_time = now;
